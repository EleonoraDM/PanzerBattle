...
public class PanzerBattleOperator implements BattleOperator {
    ...
    public String battle(Vehicle attacker, Vehicle target) {
        ...
        while (isSomeoneDead) {
            if (isAttackerTurn) {
                targetHitPoints -=
                        (long) Math.max(0, Math.ceil(attackerAttack - (targetDefense + (targetWeight / 2))));
                isAttackerTurn = false;
            } else {
                attackerHitPoints -=
                        (long) Math.max(0, Math.ceil(targetAttack - (attackerDefense + (attackerWeight / 2))));
                isAttackerTurn = true;
            }
            isSomeoneDead = isDead(attackerHitPoints) || isDead(targetHitPoints);
        }
        String battleResult = getBattleResult(attacker, target, attackerHitPoints);

        return battleResult;
    }

    private String getBattleResult(Vehicle attacker, Vehicle target, long attackerHitPoints) {
        String result;
